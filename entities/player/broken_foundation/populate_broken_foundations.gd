...
@export var _foundation_type := TowerFoundation2D.FoundationType.NA
...
func activate(parent_display_info: DisplayHelper) -> void: 
	if _count > 0:
		push_error("Something activated BrokenFoundationSpawner more than once.")
		return
...
func __populate_foundation(global_position_: Vector2) -> void:
	var new_ = load(BrokenFoundation2D.SCENE_PATH).instantiate() as BrokenFoundation2D
