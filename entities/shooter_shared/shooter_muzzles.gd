...
func get_muzzle_locations(order: MuzzleOrder = MuzzleOrder.SINGLE) -> Array[Vector3]:
	match order:
...
func _get_muzzle_location(index := 0) -> Array[Vector3]:
	return [get_child(index).get_global_position()]

func _get_all_muzzle_locations() -> Array[Vector3]:
	var out : Array[Vector3]
	for each in get_children():
		out.append(each.get_global_position())
	return out

func _get_muzzle_location_series() -> Array[Vector3]:
	var out : Array[Vector3]
